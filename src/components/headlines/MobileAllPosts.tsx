...
type Props = {
  categoryId?: number; // optional – if provided, filter by category
};
...
export default function MobileAllPosts({ categoryId }: Props) {
  ...
  useEffect(() => {
    ...
    async function fetchPosts() {
      try {
        ...
        let wpPath = `/wp-json/wp/v2/posts?per_page=${PER_PAGE}&page=${page}&orderby=date&order=desc&status=publish`;
  ...
  }, [page, hasMore, start, stop, categoryId]); // ← added categoryId to deps
