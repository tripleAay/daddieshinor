...
/* ────────────────────────────────────────────────
   MAIN COMPONENT – BRANDS (TECH STRUCTURE)
──────────────────────────────────────────────── */
    ...
    async function fetchBrands() {
      ...
      try {
        ...
        const res = await fetch(
          `${WP_BASE_URL}/wp-json/wp/v2/posts?_embed&status=publish&categories=${BRANDS_CATEGORY_ID}&per_page=7&orderby=date&order=desc`,
          { cache: "no-store" }
        );
        ...
        const mapped: BrandPost[] = (Array.isArray(data) ? data : []).map(
          (post: any) => {
            ...
            return {
              ...
              alt: title,
              brand: post?.acf?.brand_name || "Brand",
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm md:text-base text-zinc-600 dark:text-zinc-400 max-w-lg">
              Brand moves, design decisions, and strategy shifts — without fluff.
            </p>
          </div>
